...
describe('Bostadskalkyl — live Bolånkoll mortgage comparison (plan 125)', () => {
  ...
  it('auto-loads a read-only current leg without persisting or replacing Plan 118 currentMortgage', async () => {
    ...
    expect(await screen.findByTestId('current-mortgage-info')).toBeTruthy()
    ...
    // Provenance now lives behind the info popover (source · freshness ·
    // amortisation basis), keeping the column lean; open it to assert.
    const user = userEvent.setup()
    await user.click(screen.getByTestId('current-mortgage-info'))
    const info = await screen.findByTestId('current-mortgage-info-popover')
    expect(info).toHaveTextContent('Uppdaterad nu')
    expect(info).toHaveTextContent(/amortering hittad/)
  ...
  it('retries unavailable data and discards a superseded household-scope request', async () => {
    ...
    expect(await screen.findByTestId('current-mortgage-rate')).toBeTruthy()
