...
// Idempotent — safe to run on localStorage blobs and (later) cloud blobs alike.
...
// Read + forward-migrate the saved budget. Async (Promise-wrapped) so the cloud
// swap in 16d-commit-2 is a body change only — every caller already awaits.
export function loadBudget(): Promise<BudgetState | null> {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return Promise.resolve(null)
    return Promise.resolve(_migrate(JSON.parse(raw)))
  } catch {
    return Promise.resolve(null)
  }
}

export function saveBudget(state: BudgetState): Promise<void> {
